...
Ext.onReady(function() {
	var menu = [{
		...
		handler : function() {
			...
			document.excelForm.show_id.value = show_id;
			document.excelForm.obj_id.value = obj_id;
...
function queryResult() {
	...
	var param = "?project_id=" + projectID + "&month_id=" + monthID
			...
			+ "&show_id=" + show_id + "&obj_id="+ obj_id + "&time_id="+ time_id
			+ "&obj_cate_id=" + obj_cate_id + "&monthName=" + encodeURI(encodeURI(monthName))
			+ "&projectName=" + encodeURI(encodeURI(projectName));
	dhtmlGrid.load(pathUrl + '/bscresult_scoreDhtmlSubExt.action' + param, function() {
		mask.hide();
...
//	var id = ids.split('.').join(',');
	window.location = 'bsc_proj_obj_index_score_ext.jsp?month_id=' + monthID
			+ '&project_id=' + projectID + '&objCateId=' + obj_cate_id
			+ '&obj_id=' + obj_id + '&show_id=' + show_id
