  ...
  <head>
	...
	<script type="text/javascript">
		...
		var projectID="${param.project_id}",monthID = "${param.month_id}",
			...
			show_id = "${param.show_id}" ,obj_id = "${param.obj_id}" ,time_id = "${param.time_id}",
			full_score = "${param.full_score}";
		if(obj_id){ obj_id = obj_id.replace('.',',');}
		if(time_id){ time_id = time_id.replace('.',',');}
    </script>
	<script type="text/javascript" src="${pageContext.request.contextPath}/bsc/scripts/bsc_proj_obj_index_score_ext_detail_layout.js"></script>
	
  </head>
  
  <body>
  	<!-- 导出Excel文件Form -->
	<form id="excelForm" name="excelForm" action="bscresult_exportScoreSubExt.action" method="post" target="">
