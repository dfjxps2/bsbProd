  ...
  <head>
	...
	<script type="text/javascript">
		...
		var projectID="${param.project_id}",monthID = "${param.month_id}",
			objectID = "${param.object_id}",obj_cate_id="${param.obj_cate_id}",
			monthName = "${param.monthName}",projectName = "${param.projectName}",
			cycle_type_id = "${param.cycle_type_id}";
		var full_score = "${param.full_score}";
    </script>
	
	<script type="text/javascript" src="${pageContext.request.contextPath}/bsc/scripts/bsc_score_result_detail_layout.js"></script>
	
  </head>
  
  <body>
  	<!-- 导出Excel文件Form -->
		<form id="excelForm" name="excelForm" action="bsccard_export.action" method="post" target="">
