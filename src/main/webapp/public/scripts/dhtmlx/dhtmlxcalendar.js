...
dhtmlxCalendarObject.prototype = {
    ...
    drawHeader : function() {
        if (this._dblC || !this.options.isWinHeader || !this.winHeader)
            return;
        var self = this;
    ...
    getFormatedDate : function(dateformat, date, conInd) {
        ...
        if (!date)
            date = this.selDate[this._activeConInd(conInd)];
        date = new Date(date);
        var out = '';
        var plain = true;
    ...
    setFormatedDate : function(dateformatarg, date, conInd) {
        if (!date || !typeof date == 'string' || this.getFormatedDate(dateformatarg).length != date.length)
            return date;
        if (self.time) {
...
dhtmlxCalendarObject.prototype.onYearSelect = function(value) {
    if (!isNaN(value)) {
        this._c.date[this._c._activeConInd()].setFullYear(Math.min(Math.max(value, this._c.allYears[0]), this._c.allYears.slice(-1)))
    };
    this._c.draw();
    return (!isNaN(value))
};
        ...
        for (m in dhtmlXTimePicker.prototype)
            (function(m) {
                if (!dhtmlxCalendarObject.prototype[m])
                    dhtmlxCalendarObject.prototype[m] = function() {
