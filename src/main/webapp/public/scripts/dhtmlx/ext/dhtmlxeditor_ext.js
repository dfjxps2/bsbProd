dhtmlXEditor.prototype.initDhtmlxToolbar = function() {
    ...
    for (var q = 0; q < this._availFonts.length; q++) {
        ...
        this._xmlFonts += '<item type="button" id="applyFontFamily:' + fnt + '"><itemText><![CDATA[<img src="' + this.tb.imagePath + 'font_' + String(fnt).toLowerCase() + '.gif" border="0" style="width:110px;height:16px;">]]></itemText></item>'
    };
    this._availSizes = {
        "1" : "8pt",
        "2" : "10pt",
    ...
    this._ajaxOnLoad = function(obj, a, b, c, loader) {
        if (loader.xmlDoc.responseText)
            obj.setContent(loader.xmlDoc.responseText)
    };
    ...
    this.setStyleProperty = function(el, prop) {
        ...
        if (n && (n.tagName.toLowerCase() == prop))
            this.style[prop] = true;
        if (prop == "del")
