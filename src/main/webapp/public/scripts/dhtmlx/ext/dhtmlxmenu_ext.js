...
dhtmlXMenuObject.prototype.setHotKey = function(id, hkey) {
    ...
    var hkObj = null;
    for (var q = 0; q < this.idPull[id].childNodes.length; q++) {
        if (this.idPull[id].childNodes[q].tagName == "DIV" && this.idPull[id].childNodes[q].className == "dhtmlxMenu_SubLevelArea_Item_HotKey") {
...
dhtmlXMenuObject.prototype.setRTL = function(state) {
    ...
    for (var a in this.idPull) {
        if (String(a).search("polygon_") == 0) {
            this.idPull[a].className = "dhtmlxMenu_" + this.skin + "_SubLevelArea_Polygon_" + (this._rtl ? "right" : "left")
        }
    };
    for (var a in this.itemPull) {
        if (this.itemPull[a]["parent"] == this.idPrefix + this.topId) {
            if ((this.itemPull[a]["state"] == "enabled" ? this.itemPull[a]["imgen"] : this.itemPull[a]["imgdis"]) != "") {
                ...
                for (var q = 0; q < this.idPull[a].childNodes.length; q++) {
                    if (String(this.idPull[a].childNodes[q].tagName).toLowerCase() == "img") {
                        this.idPull[a].childNodes[q].className = "dhtmlxMenu_TopLevel_Item_Icon_" + (this._rtl ? "right" : "left")
                    }
                }
            }
        };
    ...
    if (this._topText != null) {
        this._topText.className = "dhtmlxMenu_TopLevel_Text_" + (this._rtl ? "left" : "right")
    };
...
dhtmlXMenuObject.prototype.setHref = function(itemId, href, target) {
    if (this.itemPull[this.idPrefix + itemId] == null)
        return;
    this.itemPull[this.idPrefix + itemId]["href_link"] = href;
    if (target != null)
        this.itemPull[this.idPrefix + itemId]["href_target"] = target
};
...
dhtmlXMenuObject.prototype._addUpArrow = function(id) {
    ...
    arrow.onmouseover = function() {
        ...
        if (this.className == "dhtmlxMenu_" + main_self.skin + "_SubLevelArea_ArrowUp_Disabled")
            return;
...
dhtmlXMenuObject.prototype._addDownArrow = function(id) {
    ...
    arrow.onmouseover = function() {
        ...
        main_self._clearAllSelectedSubItemsInPolygon(this.pId);
        if (this.className == "dhtmlxMenu_" + main_self.skin + "_SubLevelArea_ArrowDown_Disabled")
            return;
...
dhtmlXMenuObject.prototype.addRadioButton = function(mode, nextToId, pos, itemId, itemText, group, state, disabled) {
    if (this.context && nextToId == this.topId) {
        ...
        if (this.itemPull[this.idPrefix + nextToId] == null)
            return;
        if (mode == "child" && this.itemPull[this.idPrefix + nextToId]["type"] != "item")
            return
    };
    ...
    if (state == true)
        this.setRadioChecked(gr, String(id).replace(this.idPrefix, ""))
};
    ...
    for (var a in this.itemPull) {
        if (this.itemPull[a]["parent"] == parentId) {
            ...
            if (this.itemPull[a]["type"] == "item") {
                if (this.itemPull[a]["imgen"] != "")
                    imgEn = ' img="' + this.itemPull[a]["imgen"] + '"';
                if (this.itemPull[a]["imgdis"] != "")
                    imgDis = ' imgdis="' + this.itemPull[a]["imgdis"] + '"';
                if (this.itemPull[a]["hotkey"] != "")
                    hotKey = '<hotkey>' + this.itemPull[a]["hotkey"] + '</hotkey>'
            };
            if (this.itemPull[a]["type"] == "separator") {
                itemType = ' type="separator"'
            } else {
                if (this.itemPull[a]["state"] == "disabled")
