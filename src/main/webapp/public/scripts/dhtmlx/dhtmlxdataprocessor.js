...
dataProcessor.prototype = {
    ...
    _beforeSendData : function(data, rowId) {
        if (!this.callEvent("onBeforeUpdate", [rowId, this.getState(rowId)]))
            return false;
    ...
    _sendData : function(a1, rowId) {
        ...
        if (!this.callEvent("onBeforeDataSending", rowId ? [rowId, this.getState(rowId)] : []))
            return false;
    ...
    sendAllData : function() {
        ...
        if (!valid && !this.callEvent("onValidatationError", ["", this.messages]))
            return false;
        if (this._tSend)
    ...
    _getAllData : function(rowId) {
        ...
        for (var i = 0; i < this.updatedRows.length; i++) {
            ...
            if (this._in_progress[id] || this.is_invalid(id))
                continue;
            if (!this.callEvent("onBeforeUpdate", [id, this.getState(id)]))
                continue;
