...
function dhtmlXColorPicker(container, isClickOnly, customColors, hide, fullview) {
    ...
    if (window.dhtmlxColorPickerObjects)
        window.dhtmlxColorPickerObjects[window.dhtmlxColorPickerObjects.length] = this;
    else
        window.dhtmlxColorPickerObjects = [this];
    if (!window.dhtmlxColorPickerLangModules)
        window.dhtmlxColorPickerLangModules = {};
...
dhtmlXColorPicker.prototype.clickOk = function(ev) {
    ...
    if (z.onSelectHandler)
        z.onSelectHandler.apply(z, t);
...
dhtmlXColorPicker.prototype._calculateColor = function() {
    ...
    var sat = this.s;
    var lum = 1 - this.v;
    var rgb = this._hsv2rgb(hue, sat, lum);
    this.r = rgb[0];
    this.g = rgb[1];
    this.b = rgb[2];
    this.hex = this._getColorHEX(rgb);
    var rgb = '#' + this._getColorHEX(rgb);
    this.elements['cs_EndColor'].style.backgroundColor = rgb;
    this._drawValues();
    return [hue, sat, lum]
};
