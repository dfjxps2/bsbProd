...
dtmlXMLLoaderObject.prototype.getXMLTopNode = function(tagName, oldObj) {
    ...
    if(this.xmlDoc.responseText){
        ...
        var oldObj = parser.parseFromString(xmlString, "text/xml");
        this._responseXML = oldObj;
        var z = oldObj.documentElement;
        if (z){
            this._retry = false;
            return z
        }
...
dtmlXMLLoaderObject.prototype.loadXML = function(filePath, postMode, postVars, rpc) {
    ...
    this.xmlDoc.open(postMode ? "POST" : "POST", filePath, this.async);
    if (rpc) {
...
dtmlXMLLoaderObject.prototype.doXPath = function(xpathExp, docObj, namespace, result_type) {
    ...
    if (_isIE) {
        ...
        if (!docObj) {
            if (!this.xmlDoc.nodeName) {
                docObj = this.xmlDoc.responseXML || this.parseXML()
            } else {
                docObj = this.xmlDoc
            }
        };
...
dtmlXMLLoaderObject.prototype.parseXML = function() {
	try{
        ...
        if(window.DOMParser){
            ...
            var oldObj = new window.ActiveXObject("Microsoft.XMLDOM");
            oldObj.async = "false";
            oldObj.loadXML(xmlString);
            this._responseXML = oldObj;
        }
