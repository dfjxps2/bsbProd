...
function treeMenuDisplay(childWin) {
	...
	if(treeMenuCss)
		...
		treeMenuDocument.writeln('.lock_tr{z-index:200;position: relative; top: expression(this.offsetParent.scrollTop);}');
		treeMenuDocument.writeln('.lock_td{z-index:20;background:#DFDFFF;position: relative; left: expression(this.parentElement.offsetParent.parentElement.scrollLeft);}');
		treeMenuDocument.writeln('.lock_tr_td{z-index:500;background:transparent url(./public/skin/images/silverCherry/white-top-bottom.gif) repeat-x 0 -1px;position: relative; top: expression(this.offsetParent.scrollTop); left: expression(document.getElementById("lock_div").scrollLeft);}');
		treeMenuDocument.writeln('.lock_div{border-width:0px;border-style:solid;height:100%;width:100%;overflow:auto;cursor:default;}');
	...
	treeMenuDocument.writeln('<div id=\"lock_div\" class=lock_div>');
	if(doXmlLoader == true)
...
//	treeMenuDocument.writeln('<div id=\"lock_div_body\">');
...
function treeMenuListItems(menu) {
	...
	if(menu==null || menu.items==null)
		return;
//	treeMenuDocument.writeln('<div id=\"lock_div_body\" class=lock_div>'); aaaaaaaaaaa
...
function getMenuItemTr(item){
	...
	if(isLockColumn)
		trHead = '<tr valign=top id='+ item.objId + '><td nowrap class=lock_td>';
	else
		trHead = '<tr valign=top id='+ item.objId + '><td nowrap>';//aaaaaaaaaaa
...
/*
	...
	trHtml += '<span style="FONT-SIZE:' 
			...
			+ treeMenuFont + '">';
	if(item.url!="")
		trHtml+='<a href=javascript:'+item.url+'><span style="text-decoration : underline;cursor: hand">'+item.text+'</span>';
	else
	    trHtml += item.text;
	if(item.url!="")
		trHtml+='</a>';
	...
	*/
	...
	if(doXmlLoader)
	{
		...
		for(var i=1;i<nodeList.length;i++)
		{
			...
			if(href!="")
				trHtml+='<a href="'+href+'"><span style="text-decoration : underline;cursor: hand">'+text+'</span>';
			else
				trHtml+= text ;
			if(href!="")
				trHtml+='</a>';
...
function _dt_createItemTR(item){
	...
	//alert("<div class=lock_div><table>" + getMenuItemTr(item) + "</table><div>")bbbbbb
