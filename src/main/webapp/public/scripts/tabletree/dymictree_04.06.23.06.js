...
var treeMenuIndex = 0;	// 全局 index 变量.
var treeMenu;

var treeMenuName	= "";
var treeMenuDays	= 1;  //Cokie有效天数
 ...
 * 每个ITEM需要两个id
 * DbId：数据库中的节点id
 * ObjId：页面中的节点id， 例如 <div id='$ObjId'>
 * cookieName: cookie中使用的名称
 * 例如：typeName如果是　com.ezkj.system.user, dbid=220
 *  那么，DbId=220, ObjId=user_220，cookieName=user_220
 */
...
function treeMenuDisplay(childWin) {
	...
	if(treeMenuCss)
		...
		treeMenuDocument.writeln('.root{white-space:nowrap;cursor:hand;font:10pt 宋体;font-weight:700;color:black;padding-top:2px;height:12px;}');
				treeMenuDocument.writeln('.selroot{background-color:' + treeMenuHiBg + ';color:' + treeMenuHiFg + ';border:2px dashed yellow;white-space:nowrap;cursor:hand;font:10pt 宋体;font-weight:700;padding-top:2px;height:12px;}');
		treeMenuDocument.writeln('</style>');
	}
...
/*
	...
	*/
	...
	if(doXmlLoader)
	{
		var node=document.all.xmldata.XMLDocument.documentElement.selectSingleNode("//node[@p0=\"" + item.objId + "\"]");
		for(var i=3;i<node.attributes.length;i++)
		{
			trHtml+='<td nowrap align="' + (xmlTableTitleAlign[i-1]!=null?xmlTableTitleAlign[i-1]:"left") + '">' + node.attributes.item(i).value + '</td>'; 
		}
	}
	trTail = '</tr>';
