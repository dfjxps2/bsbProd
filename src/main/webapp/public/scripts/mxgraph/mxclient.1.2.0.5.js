var mxClient = {
	...
	IS_IE7 : navigator.userAgent.indexOf('MSIE 7') >= 0 || navigator.userAgent.indexOf('MSIE 8') >= 0,
...
if (typeof(mxImageBasePath) != 'undefined' && mxImageBasePath.length > 0) {
	if (mxImageBasePath.substring(mxImageBasePath.length - 1) == '/') {
		mxImageBasePath = mxImageBasePath.substring(0, mxImageBasePath.length
				- 1);
	}
...
mxXmlRequest.prototype.send = function(onload, onerror) {
	...
	if (this.request != null) {
		...
		//alert(this.method+' '+this.url+' '+this.async+' '+this.username+' '+this.password);
...
function mxWindow(title, content, x, y, width, height, minimizable, movable,
		replaceNode, style) {
	if (content != null) {
...
mxWindow.prototype.activate = function() {
	if (mxWindow.activeWindow != this) {
		...
		this.fireEvent(new mxEventObject(mxEvent.ACTIVATE, 'previousWindow',
				previousWindow));
	}
};
	...
	var funct = mxUtils.bind(this, function(evt) {
		...
		if (this.maximize.style.display != 'none') {
			if (!maximized) {
				...
				if (!true) {
					this.div.style.height = (document.body.clientHeight - 2)
							+ 'px';
					this.div.style.width = (document.body.clientWidth - 2)
							+ 'px';
				}
				this.table.style.width = (document.body.clientWidth - 2) + 'px';
				this.table.style.height = (document.body.clientHeight - 2)
						+ 'px';
				if (this.resize != null) {
					this.resize.style.visibility = 'hidden';
				}
				...
				this
						.fireEvent(new mxEventObject(mxEvent.MAXIMIZE, 'event',
								evt));
			} else {
				maximized = false;
				this.maximize.setAttribute('src', this.maximizeImage);
				...
				this.updateShadow();
				this.fireEvent(new mxEventObject(mxEvent.NORMALIZE, 'event',
						evt));
			}
			mxEvent.consume(evt);
		}
...
mxWindow.prototype.installMoveHandler = function() {
	...
	mxEvent.addListener(this.title, 'mousedown', mxUtils.bind(this, function(
			evt) {
		var startX = evt.clientX;
		var startY = evt.clientY;
		var x = this.getX();
...
function mxDivResizer(div, container) {
	if (div.nodeName.toLowerCase() == 'div') {
		...
		mxEvent.addListener(container, 'resize', mxUtils.bind(this, function(
				evt) {
...
mxToolbar.prototype.addSwitchMode = function(title, icon, funct, pressedIcon,
		style) {
	var img = document.createElement('img');
...
mxSession.prototype.notify = function(xml, onLoad, onError) {
	if (xml != null && xml.length > 0) {
		...
		this.fireEvent(new mxEventObject(mxEvent.NOTIFY, "url", this.urlNotify,
				"xml", xml));
	}
};
			...
			if (typeof(mxUtils) != 'undefined') {
				try {
					if (req.isReady() && req.getStatus() != 404) {
						...
						this.fireEvent(new mxEventObject(mxEvent.GET, 'url',
								url, 'request', req));
	...
	if (node != null && node.nodeType == mxConstants.NODETYPE_ELEMENT) {
		...
		if (name == 'state') {
			...
			if (changes.length > 0) {
				...
				this.model.fireEvent(new mxEventObject(mxEvent.UNDO, 'edit',
						edit));
				this.model.fireEvent(new mxEventObject(mxEvent.CHANGE,
						'changes', changes));
				this.fireEvent(new mxEventObject(mxEvent.FIRED, 'changes',
						changes));
			}
		}
...
mxSession.prototype.createUndoableEdit = function(changes) {
	...
	edit.notify = function() {
		edit.source.fireEvent(new mxEventObject(mxEvent.CHANGE, 'changes',
				edit.changes));
		edit.source.fireEvent(new mxEventObject(mxEvent.NOTIFY, 'changes',
				edit.changes));
	};
	return edit;
};
...
mxPath.prototype.curveTo = function(x1, y1, x2, y2, x, y) {
	if (this.isVml()) {
		this.path.push('c ', Math.floor(this.translate.x + x1), ' ', Math
				.floor(this.translate.y + y1), ' ', Math.floor(this.translate.x
				+ x2), ' ', Math.floor(this.translate.y + y2), ' ', Math
				.floor(this.translate.x + x), ' ', Math.floor(this.translate.y
				+ y), ' ');
	} else {
...
mxShape.prototype.apply = function(state) {
	...
	if (style != null) {
		...
		this.opacity = mxUtils.getValue(style, mxConstants.STYLE_OPACITY,
				this.opacity);
		this.stroke = mxUtils.getValue(style, mxConstants.STYLE_STROKECOLOR,
				this.stroke);
		...
		this.spacing = mxUtils.getValue(style, mxConstants.STYLE_SPACING,
				this.spacing);
		this.startSize = mxUtils.getValue(style, mxConstants.STYLE_STARTSIZE,
				this.startSize);
		...
		this.isRounded = mxUtils.getValue(style, mxConstants.STYLE_ROUNDED,
				this.isRounded);
		this.startArrow = mxUtils.getValue(style, mxConstants.STYLE_STARTARROW,
				this.startArrow);
...
mxShape.prototype.configureVmlShape = function(node) {
	...
	if (node.nodeName == 'roundrect') {
		try {
			node.setAttribute('arcsize',
					String(mxConstants.RECTANGLE_ROUNDING_FACTOR * 100) + '%');
		} catch (e) {
		}
	}
};
...
mxShape.prototype.updateHtmlShape = function(node) {
	if (node != null) {
		if (mxUtils.isVml(node)) {
			...
			node.style.borderWidth = Math.max(1, Math.floor(this.strokewidth
					* this.scale))
					+ 'px';
			if (this.bounds != null) {
		...
		if (node.nodeName == 'polyline' && node.points != null) {
			...
			if (this.style != null && this.style[mxConstants.STYLE_SMOOTH]) {
				...
				if (n > 3) {
					...
					points = 'm ' + Math.floor(pts[0].x - x0) + ' '
							+ Math.floor(pts[0].y - y0) + ' qb';
					for (var i = 1; i < n - 1; i++) {
...
mxShape.prototype.updateSvgShape = function(node) {
	...
	if (this.points != null && this.points[0] != null) {
		...
		if (this.isRounded) {
			var r = Math.min(w * mxConstants.RECTANGLE_ROUNDING_FACTOR, h
...
mxShape.prototype.updateSvgTransform = function(node, shadow) {
	if (this.rotation != null && this.rotation != 0) {
		...
		if (shadow) {
			node.setAttribute('transform', 'rotate(' + this.rotation + ',' + cx
					+ ',' + cy + ') ' + mxConstants.SVG_SHADOWTRANSFORM);
		} else {
			node.setAttribute('transform', 'rotate(' + this.rotation + ',' + cx
					+ ',' + cy + ')');
		}
...
function mxText(value, bounds, align, valign, color, family, size, fontStyle,
		spacing, spacingTop, spacingRight, spacingBottom, spacingLeft,
		isRotate, background, border, useTableBounds, isAbsolute, isWrapping,
		isClipping) {
	this.value = value;
	this.bounds = bounds;
...
mxText.prototype.redrawVml = function() {
	if (this.value != null) {
		...
		td.style.textAlign = (this.align == mxConstants.ALIGN_RIGHT)
				? 'right'
				: ((this.align == mxConstants.ALIGN_CENTER) ? 'center' : 'left');
		td.style.verticalAlign = (this.valign == mxConstants.ALIGN_BOTTOM)
				? 'bottom'
				: ((this.valign == mxConstants.ALIGN_MIDDLE) ? 'middle' : 'top');
		var container = td;
		...
		if (this.node.style.overflow != 'hidden') {
			...
			if (xdiff > 0 || ydiff > 0) {
				...
				var y0 = parseInt(this.node.parentNode.style.top);
				this.node.inset = (this.bounds.x - x0) + 'px,'
						+ (this.bounds.y - y0) + 'px,' + (y0 - this.bounds.y)
						+ 'px,' + (x0 - this.bounds.x) + 'px';
			}
		}
		if (this.opacity != null) {
...
mxText.prototype.redrawSvg = function() {
	...
	var y0 = (this.valign == mxConstants.ALIGN_BOTTOM)
			? ((this.isRotate) ? this.bounds.width : this.bounds.height)
					- (lineCount - 1) * dy - this.spacingBottom * this.scale
					- 3
			: (this.valign == mxConstants.ALIGN_MIDDLE)
					? (this.spacingTop
							* this.scale
	...
	if (!false && this.value != null && this.value.length > 0) {
		try {
			...
			this.boundingBox = new mxRectangle(Math.min(this.bounds.x, box.x
					- 4 * this.scale || 0), Math.min(this.bounds.y, box.y - 4
					* this.scale || 0), Math.max(this.bounds.width, box.width
					+ 8 * this.scale || 0), Math.max(this.bounds.height,
			...
			if (this.backgroundNode != null && this.node.firstChild != null) {
				...
				this.backgroundNode.setAttribute('x', box.x - 4 * this.scale
						|| 0);
				this.backgroundNode.setAttribute('y', box.y - 4 * this.scale
						|| 0);
...
mxLabel.prototype.createSvg = function() {
	...
	if (this.indicatorImage != null) {
		this.indicatorImageNode = document.createElementNS(mxConstants.NS_SVG,
				'image');
		this.indicatorImageNode.setAttributeNS(mxConstants.NS_XLINK, 'href',
				this.indicatorImage);
...
mxLabel.prototype.redraw = function() {
	...
	if (this.indicator != null) {
		this.indicator.bounds = new mxRectangle(x + (width - indicatorWidth)
				/ 2, y + imageHeight + indicatorSpacing, indicatorWidth,
				indicatorHeight);
		...
		if (isSvg) {
			...
			this.indicatorImageNode
					.setAttribute('width', indicatorWidth + 'px');
...
mxConnector.prototype.redrawVml = function() {
	if (this.node != null && this.strokeNode != null) {
		var startSize = mxUtils.getValue(this.style,
				mxConstants.STYLE_STARTSIZE, mxConstants.DEFAULT_MARKERSIZE)
				* this.scale;
...
mxConnector.prototype.redrawSvg = function() {
	...
	var strokeWidth = this.strokewidth * this.scale;
	var color = this.innerNode.getAttribute('stroke');
	if (mxConstants.SVG_CRISP_EDGES && strokeWidth == Math.floor(strokeWidth)
			&& !this.isRounded) {
	...
	if (this.points != null && this.points[0] != null) {
		...
		if (this.end != null) {
			...
			var size = mxUtils.getValue(this.style, mxConstants.STYLE_ENDSIZE,
					mxConstants.DEFAULT_MARKERSIZE);
			this.endOffset = this.redrawSvgMarker(this.end, this.endArrow, p0,
					pe, color, size);
		}
	}
	this.updateSvgShape(this.innerNode);
...
mxConnector.prototype.redrawSvgMarker = function(node, type, p0, pe, color,
	...
	if (type == 'classic' || type == 'block') {
		...
		var d = 'M ' + (pe.x - ny / 2) + ' ' + (pe.y + nx / 2) + ' a '
				+ (absSize / 2) + ' ' + (absSize / 2) + ' 0  1,1 ' + (nx / 8)
				+ ' ' + (ny / 8) + ' z';
		node.setAttribute('d', d);
...
mxSwimlane.prototype.reconfigure = function(node) {
	...
	if (this.dialect == mxConstants.DIALECT_SVG) {
		if (this.shadowNode != null) {
			this.updateSvgShape(this.shadowNode);
			if (mxUtils
					.getValue(this.style, mxConstants.STYLE_HORIZONTAL, true)) {
...
mxSwimlane.prototype.redrawHtml = function() {
	...
	if (mxUtils.getValue(this.style, mxConstants.STYLE_HORIZONTAL, true)) {
		...
		if (this.separator != null) {
			...
			this.separator.style.top = Math.floor(this.startSize * this.scale)
					+ 'px';
		...
		if (this.imageNode != null) {
			...
			this.imageNode.style.width = Math
					.floor(this.imageSize * this.scale)
					+ 'px';
		...
		if (this.separator != null) {
			this.separator.style.left = Math.floor(this.startSize * this.scale)
					+ 'px';
...
mxSwimlane.prototype.redrawSvg = function() {
	...
	var horizontal = mxUtils.getValue(this.style, mxConstants.STYLE_HORIZONTAL,
			true);
	if (this.shadowNode != null) {
		this.updateSvgShape(this.shadowNode);
		if (horizontal) {
	...
	if (horizontal) {
		...
		var h = this.startSize * this.scale;
		var points = 'M ' + this.bounds.x + ' ' + (this.bounds.y + h) + ' l 0 '
				+ (this.bounds.height - h) + ' l ' + this.bounds.width + ' 0'
		...
		if (this.separator != null) {
			this.separator
					.setAttribute('x1', this.bounds.x + this.bounds.width);
			this.separator.setAttribute('y1', this.bounds.y + this.startSize
					* this.scale);
			this.separator
					.setAttribute('x2', this.bounds.x + this.bounds.width);
			this.separator.setAttribute('y2', this.bounds.y
					+ this.bounds.height);
		}
		if (this.imageNode != null) {
			this.imageNode.setAttribute('x', this.bounds.x + this.bounds.width
					- this.imageSize - 4);
			this.imageNode.setAttribute('y', this.bounds.y);
			this.imageNode.setAttribute('width', this.imageSize * this.scale
					+ 'px');
			this.imageNode.setAttribute('height', this.imageSize * this.scale
					+ 'px');
		}
	} else {
		this.startSize = Math.min(this.startSize, this.bounds.width);
		...
		var w = this.startSize * this.scale;
		var points = 'M ' + (this.bounds.x + w) + ' ' + this.bounds.y + ' l '
				+ (this.bounds.width - w) + ' 0' + ' l 0 ' + this.bounds.height
		...
		this.content.removeAttribute('height');
		if (this.separator != null) {
			this.separator.setAttribute('x1', this.bounds.x + this.startSize
					* this.scale);
...
mxStackLayout.prototype.execute = function(parent) {
	if (parent != null) {
		...
		var size = (this.graph.isSwimlane(parent)) ? this.graph
				.getStartSize(parent) : new mxRectangle();
				...
				if (!this.isVertexIgnored(child) && this.isVertexMovable(child)) {
					...
					if (geo != null) {
						...
						if (this.wrap != null && last != null) {
							if ((horizontal && last.x + last.width + geo.width
									+ 2 * this.spacing > this.wrap)
...
mxCompactTreeLayout.prototype.execute = function(parent, root) {
	var model = this.graph.getModel();
	if (root == null) {
		if (this.graph.getEdges(parent, model.getParent(parent), this.invert,
				!this.invert, false).length > 0) {
			root = parent;
		} else {
			...
			if (roots.length > 0) {
				for (var i = 0; i < roots.length; i++) {
					if (!this.isVertexIgnored(roots[i])
							&& this.graph.getEdges(roots[i], null, this.invert,
									!this.invert, false).length > 0) {
						root = roots[i];
						break;
					}
...
mxCompactTreeLayout.prototype.dfs = function(cell, parent, visited) {
	...
	if (cell != null && visited[id] == null && !this.isVertexIgnored(cell)) {
		...
		var model = this.graph.getModel();
		var prev = null;
		var out = this.graph.getEdges(cell, parent, this.invert, !this.invert,
				false);
...
mxCompactTreeLayout.prototype.verticalLayout = function(node, parent, x0, y0,
		bounds) {
...
mxCompactTreeLayout.prototype.attachParent = function(node, height) {
	...
	node.contour.upperHead = this.createLine(node.height, 0, this.createLine(x,
			y1, node.contour.upperHead));
	node.contour.lowerHead = this.createLine(node.height, 0, this.createLine(x,
			y2, node.contour.lowerHead));
};
		...
		for (var j = 0; j < outgoingCells.length; j++) {
			...
			if (cell != vertices[i] && !layout.isVertexIgnored(cell)) {
				...
				if (edges != null && edges.length > 0) {
					...
					if (mxUtils.indexOf(internalVertices[i].connectsAsSource,
							internalEdge) < 0) {
...
mxGraphHierarchyModel.prototype.dfs = function(parent, root, connectingEdge,
		visitor, seen, layer) {
	if (root != null) {
		...
		if (seen[rootId] == null) {
			...
			for (var i = 0; i < root.connectsAsSource.length; i++) {
				...
				this.dfs(root, targetNode, internalEdge, visitor, seen, layer
						+ 1);
			}
		} else {
...
mxMedianHybridCrossingReduction.prototype.calculateRankCrossing = function(i,
		model) {
	var totalCrossings = 0;
			...
			if (weightedValue != null) {
				...
				if (weightedValue.visited) {
					leftLimit = leftCell.getGeneralPurposeVariable(rankValue)
							+ leftCell.width / 2.0 + this.intraCellSpacing
			...
			if (weightedValue != null) {
				...
				if (weightedValue.visited) {
					rightLimit = rightCell.getGeneralPurposeVariable(rankValue)
							- rightCell.width / 2.0 - this.intraCellSpacing
...
mxCoordinateAssignment.prototype.rankCoordinates = function(rankValue, graph,
		model) {
	var rank = model.ranks[rankValue];
	var maxY = 0.0;
...
mxCoordinateAssignment.prototype.calculateWidestRank = function(graph, model) {
	...
	for (var rankValue = model.maxRank; rankValue >= 0; rankValue--) {
		...
		var distanceToNextRank = maxCellHeight / 2.0 + lastRankMaxCellHeight
				/ 2.0 + this.interRankCellSpacing;
	...
	if (this.roots != null) {
		...
		try {
			...
			if (this.resizeParent && !this.graph.isCellCollapsed(parent)) {
				this.graph.updateGroupBounds([parent], this.parentBorder,
						this.moveParent);
			}
		} finally {
			model.endUpdate();
...
mxHierarchicalLayout.prototype.run = function(parent) {
	...
	for (var i = 0; i < hierarchyVertices.length; i++) {
		...
		this.model = new mxGraphHierarchyModel(this, tmp, this.roots, parent,
				false, this.deterministic, this.tightenToSource);
...
mxHierarchicalLayout.prototype.placementStage = function(initialX, parent) {
	var placementStage = new mxCoordinateAssignment(this,
			this.intraCellSpacing, this.interRankCellSpacing, this.orientation,
			initialX, this.parallelEdgeSpacing);
...
mxGraphModel.prototype.updateEdgeParent = function(edge, root) {
	...
	if (this.isAncestor(root, source) && this.isAncestor(root, target)) {
		...
		if (cell != null
				&& (this.getParent(cell) != this.root || this.isAncestor(cell,
						edge)) && this.getParent(edge) != cell) {
...
mxGraphModel.prototype.mergeChildrenImpl = function(from, to, cloneAllEdges,
		mapping) {
	this.beginUpdate();
	try {
...
mxCell.prototype.mxTransient = ['id', 'value', 'parent', 'source', 'target',
		'children', 'edges'];
...
function mxPrintPreview(graph, scale, pageFormat, border, x0, y0, borderColor,
		title, pageSelector) {
	this.graph = graph;
	this.scale = (scale != null) ? scale : 1;
	this.border = (border != null) ? border : 0;
	this.pageFormat = (pageFormat != null)
			? pageFormat
			: mxConstants.PAGE_FORMAT_A4_PORTRAIT;
...
mxPrintPreview.prototype.open = function(css) {
	...
	try {
		if (this.wnd == null) {
			...
			var hpages = Math.max(1, Math.ceil((bounds.width + this.y0)
					/ availableWidth));
			var vpages = Math.max(1, Math.ceil((bounds.height + this.x0)
					/ availableHeight));
			this.pageCount = hpages * vpages;
			for (var i = 0; i < vpages; i++) {
				var dy = i * availableHeight / this.scale - this.y0
						/ this.scale + (bounds.y - tr.y * currentScale)
						/ currentScale;
				for (var j = 0; j < hpages; j++) {
					if (this.wnd == null) {
						return null;
					}
					var dx = j * availableWidth / this.scale - this.x0
							/ this.scale + (bounds.x - tr.x * currentScale)
							/ currentScale;
					div = this.renderPage(this.pageFormat.width,
...
mxCellEditor.prototype.init = function() {
	...
	mxEvent.addListener(this.textarea, 'keydown', mxUtils.bind(this, function(
			evt) {
...
mxCellEditor.prototype.stopEditing = function(cancel) {
	...
	if (this.editingCell != null) {
		...
		if (!cancel && this.isModified()) {
			this.graph.labelChanged(this.editingCell, this.getCurrentValue(),
					this.trigger);
		}
...
mxCellEditor.prototype.getEditorBounds = function(state) {
	...
	var spacingTop = (parseInt(state.style[mxConstants.STYLE_SPACING_TOP] || 0))
			* scale + spacing;
...
mxCellRenderer.prototype.getPreviousStateInContainer = function(state,
		container) {
	var result = null;
...
mxCellRenderer.prototype.findPreviousStateInContainer = function(graph, cell,
		stop, container) {
	var result = null;
	var model = graph.getModel();
	if (stop != null) {
		...
		for (var i = start - 1; i >= 0 && result == null; i--) {
			result = this.findPreviousStateInContainer(graph, model.getChildAt(
					cell, i), null, container);
		}
	} else {
...
mxCellRenderer.prototype.installCellOverlayListeners = function(state, overlay,
		shape) {
	var graph = state.view.graph;
	mxEvent.addListener(shape.node, 'click', function(evt) {
		overlay.fireEvent(new mxEventObject(mxEvent.CLICK, 'event', evt,
				'cell', state.cell));
	});
...
mxCellRenderer.prototype.getControlBounds = function(state) {
	if (state.control != null) {
		...
		return (state.view.graph.getModel().isEdge(state.cell))
				...
				: new mxRectangle(state.x + w / 2 * s, state.y + h / 2 * s, w
						* s, h * s);
	}
	return null;
};
	...
	if (cells != null && cells.length > 0) {
		...
		for (var i = 0; i < cells.length; i++) {
			if (model.isVertex(cells[i]) || model.isEdge(cells[i])) {
				...
				if (state != null) {
					if (result == null) {
						result = new mxRectangle(state.x, state.y, state.width,
								state.height);
					} else {
						result.add(state);
					}
...
mxGraphView.prototype.scaleAndTranslate = function(scale, dx, dy) {
	...
	this.fireEvent(new mxEventObject(mxEvent.SCALE_AND_TRANSLATE, 'scale',
			scale, 'previousScale', previousScale, 'translate', this.translate,
...
mxGraphView.prototype.setScale = function(value) {
	...
	this.fireEvent(new mxEventObject(mxEvent.SCALE, 'scale', value,
...
mxGraphView.prototype.setTranslate = function(dx, dy) {
	...
	this.fireEvent(new mxEventObject(mxEvent.TRANSLATE, 'translate',
			this.translate, 'previousTranslate', previousTranslate));
};
...
mxGraphView.prototype.validateBackground = function() {
	...
	if (this.graph.pageVisible) {
		...
		var bounds = new mxRectangle(this.scale * this.translate.x, this.scale
				* this.translate.y, fmt.width * ps, fmt.height * ps);
		if (this.backgroundPageShape == null) {
			this.backgroundPageShape = new mxRectangleShape(bounds, 'white',
					'black');
...
mxGraphView.prototype.updateVertexLabelOffset = function(state) {
	...
	var vertical = mxUtils
			.getValue(state.style, mxConstants.STYLE_VERTICAL_LABEL_POSITION,
					mxConstants.ALIGN_MIDDLE);
	if (vertical == mxConstants.ALIGN_TOP) {
...
mxGraphView.prototype.transformControlPoint = function(state, pt) {
	...
	return new mxPoint((pt.x + this.translate.x + origin.x) * this.scale, (pt.y
			+ this.translate.y + origin.y)
			* this.scale);
};
mxGraphView.prototype.getEdgeStyle = function(edgeState, points, source, target) {
	var edgeStyle = (source != null && source == target) ? mxUtils.getValue(
			edgeState.style, mxConstants.STYLE_LOOP,
			this.graph.defaultLoopStyle) : (!mxUtils.getValue(edgeState.style,
...
mxGraphView.prototype.getNextPoint = function(state, opposite, isSource) {
	...
	if (pts != null && (isSource || pts.length > 2 || opposite == null)) {
		...
		point = pts[(isSource) ? Math.min(1, count - 1) : Math
				.max(0, count - 2)];
	}
...
mxGraphView.prototype.init = function() {
	...
	if (container != null) {
		...
		mxEvent.addListener(container, 'mouseup', mxUtils.bind(this, function(
				evt) {
			if (this.isContainerEvent(evt)) {
				graph.fireMouseEvent(mxEvent.MOUSE_UP, new mxMouseEvent(evt));
			}
		}));
		mxEvent.addListener(container, 'dblclick', mxUtils.bind(this, function(
				evt) {
			graph.dblClick(evt);
			mxEvent.consume(evt);
		}));
		...
		mxEvent.addListener(document, 'mousemove', mxUtils.bind(this, function(
				evt) {
...
mxCurrentRootChange.prototype.execute = function() {
	...
	this.view.fireEvent(new mxEventObject(name, 'root', this.view.currentRoot,
...
function mxGraph(container, model, renderHint) {
	...
	this.model.addListener(mxEvent.CHANGE, mxUtils.bind(this, function(sender,
			evt) {
		this.graphModelChanged(evt.getProperty('changes'));
	}));
	this.createHandlers();
...
mxGraph.prototype.addCellOverlay = function(cell, overlay) {
	...
	this.fireEvent(new mxEventObject(mxEvent.ADD_OVERLAY, 'cell', cell,
			'overlay', overlay));
	return overlay;
};
...
mxGraph.prototype.removeCellOverlay = function(cell, overlay) {
	if (overlay == null) {
		...
		if (index >= 0) {
			...
			this.fireEvent(new mxEventObject(mxEvent.REMOVE_OVERLAY, 'cell',
...
mxGraph.prototype.startEditingAtCell = function(cell, evt) {
	...
	if (cell != null) {
		this.fireEvent(new mxEventObject(mxEvent.START_EDITING, 'cell', cell,
				'event', evt));
...
mxGraph.prototype.labelChanged = function(cell, value, evt) {
	...
	try {
		...
		this.fireEvent(new mxEventObject(mxEvent.LABEL_CHANGED, 'cell', cell,
				'value', value, 'event', evt));
	} finally {
			...
			if (dy < border) {
				...
				if (old == c.scrollTop && extend) {
					if (this.dialect == mxConstants.DIALECT_SVG) {
						...
						var height = Math.max(c.clientHeight, c.scrollHeight)
								+ border - dy;
						var canvas = this.view.getCanvas();
...
mxGraph.prototype.orderCells = function(back, cells) {
	...
	try {
		...
		this.fireEvent(new mxEventObject(mxEvent.ORDER_CELLS, 'back', back,
				'cells', cell));
	} finally {
		this.model.endUpdate();
	}
...
mxGraph.prototype.cellsOrdered = function(cells, back) {
	if (cells != null) {
		...
		try {
			...
			this.fireEvent(new mxEventObject(mxEvent.CELLS_ORDERED, 'back',
					back, 'cells', cells));
		} finally {
			this.model.endUpdate();
		}
...
mxGraph.prototype.groupCells = function(group, border, cells) {
	...
	if (cells.length > 1 && bounds != null) {
		...
		try {
			...
			this.fireEvent(new mxEventObject(mxEvent.GROUP_CELLS, 'group',
					group, 'border', border, 'cells', cells));
		} finally {
			this.model.endUpdate();
		}
	}
...
mxGraph.prototype.ungroupCells = function(cells) {
	...
	if (cells != null && cells.length > 0) {
		...
		try {
			...
			this.fireEvent(new mxEventObject(mxEvent.UNGROUP_CELLS, 'cells',
					cells));
		} finally {
			this.model.endUpdate();
		}
			...
			if (geo != null) {
				...
				if (children != null && children.length > 0) {
					...
					if (childBounds.width > 0 && childBounds.height > 0) {
						var size = (this.isSwimlane(cells[i])) ? this
								.getStartSize(cells[i]) : new mxRectangle();
						geo = geo.clone();
						if (moveGroup) {
						...
						geo.height = childBounds.height + size.height + 2
								* border;
						this.model.setGeometry(cells[i], geo);
						this
								.moveCells(children, -childBounds.x
										+ size.width + border, -childBounds.y
										+ size.height + border);
					}
				}
			}
		...
		if (tmp.length > 0) {
			...
			for (var i = 0; i < cells.length; i++) {
				if (!allowInvalidEdges
						...
						&& this.getEdgeValidationError(clones[i], this.model
								.getTerminal(clones[i], true), this.model
...
mxGraph.prototype.addCells = function(cells, parent, index, source, target) {
	...
	try {
		...
		this.fireEvent(new mxEventObject(mxEvent.ADD_CELLS, 'cells', cells,
				'parent', parent, 'index', index, 'source', source, 'target',
				target));
	} finally {
		this.model.endUpdate();
	}
...
mxGraph.prototype.cellsAdded = function(cells, parent, index, source, target,
		absolute) {
	if (cells != null && parent != null && index != null) {
		...
		try {
			...
			this.fireEvent(new mxEventObject(mxEvent.CELLS_ADDED, 'cells',
					cells, 'parent', parent, 'index', index, 'source', source,
...
mxGraph.prototype.removeCells = function(cells, includeEdges) {
	...
	try {
		...
		this.fireEvent(new mxEventObject(mxEvent.REMOVE_CELLS, 'cells', cells,
...
mxGraph.prototype.cellsRemoved = function(cells) {
	if (cells != null && cells.length > 0) {
		...
		try {
			...
			this.fireEvent(new mxEventObject(mxEvent.CELLS_REMOVED, 'cells',
					cells));
		} finally {
			this.model.endUpdate();
		}
...
mxGraph.prototype.splitEdge = function(edge, cells, newEdge, dx, dy) {
	...
	try {
		...
		this.cellsAdded(cells, parent, this.model.getChildCount(parent), null,
				null, true);
...
mxGraph.prototype.toggleCells = function(show, cells, includeEdges) {
	...
	try {
		...
		this.fireEvent(new mxEventObject(mxEvent.TOGGLE_CELLS, 'show', show,
...
mxGraph.prototype.foldCells = function(collapse, recurse, cells) {
	...
	try {
		...
		this.fireEvent(new mxEventObject(mxEvent.FOLD_CELLS, 'collapse',
				collapse, 'recurse', recurse, 'cells', cells));
	} finally {
		this.model.endUpdate();
	}
	return cells;
...
mxGraph.prototype.updateAlternateBounds = function(cell, geo, willCollapse) {
	if (cell != null && geo != null) {
		if (geo.alternateBounds == null) {
			...
			geo.alternateBounds = new mxRectangle(geo.x, geo.y, bounds.width,
					bounds.height);
		} else {
...
mxGraph.prototype.updateCellSize = function(cell, ignoreChildren) {
	...
	try {
		...
		this.fireEvent(new mxEventObject(mxEvent.UPDATE_CELL_SIZE, 'cell',
			...
			if (size != null && geo != null) {
				...
				if (!ignoreChildren && !collapsed) {
					...
					if (bounds != null) {
						var tr = this.view.translate;
						var scale = this.view.scale;
						var width = (bounds.x + bounds.width) / scale - geo.x
								- tr.x;
						var height = (bounds.y + bounds.height) / scale - geo.y
								- tr.y;
	...
	if (cell != null) {
		...
		if (style != null && !this.model.isEdge(cell)) {
			...
			if (value != null && value.length > 0) {
				...
				if (!mxUtils
						.getValue(style, mxConstants.STYLE_HORIZONTAL, true)) {
					var tmp = height;
					height = width;
					width = tmp;
				}
...
mxGraph.prototype.resizeCells = function(cells, bounds) {
	...
	try {
		...
		this.fireEvent(new mxEventObject(mxEvent.RESIZE_CELLS, 'cells', cells,
				'bounds', bounds));
	} finally {
		this.model.endUpdate();
	}
...
mxGraph.prototype.cellsResized = function(cells, bounds) {
	if (cells != null && bounds != null && cells.length == bounds.length) {
		...
		try {
			...
			this.fireEvent(new mxEventObject(mxEvent.CELLS_RESIZED, 'cells',
					cells, 'bounds', bounds));
		} finally {
			this.model.endUpdate();
		}
...
mxGraph.prototype.cellsMoved = function(cells, dx, dy, disconnect, constrain) {
	if (cells != null && (dx != 0 || dy != 0)) {
		...
		try {
			...
			this.fireEvent(new mxEventObject(mxEvent.CELLS_MOVED, 'cells',
					cells, 'dx', dy, 'dy', dy, 'disconnect', disconnect));
		} finally {
			this.model.endUpdate();
		}
	}
...
mxGraph.prototype.constrainChild = function(cell) {
	if (cell != null) {
		...
		var area = (this.isConstrainChild(cell)) ? this
				.getCellContainmentArea(cell) : this.getMaximumGraphBounds();
		if (geo != null && area != null) {
			if (!geo.relative
				...
				if (area.width > 0) {
					geo.x = Math.min(geo.x, area.x + area.width - (1 - overlap)
							* geo.width);
				}
				if (area.height > 0) {
				...
				if (this.model.isEdge(cells[i])) {
					...
					if (geo != null) {
						...
						if (state != null && pstate != null) {
							...
							if (src != null
									&& this.isCellDisconnectable(cells[i], src,
											true)) {
								var srcId = mxCellPath.create(src);
								while (src != null && hash[srcId] == null) {
									src = this.model.getParent(src);
								...
								if (src == null) {
									geo.setTerminalPoint(new mxPoint(pts[0].x
											/ scale - tr.x + dx, pts[0].y
											/ scale - tr.y + dy), true);
									this.model
											.setTerminal(cells[i], null, true);
								}
							}
							var trg = this.model.getTerminal(cells[i], false);
							if (trg != null
									&& this.isCellDisconnectable(cells[i], trg,
											false)) {
								var trgId = mxCellPath.create(trg);
								while (trg != null && hash[trgId] == null) {
									trg = this.model.getParent(trg);
	...
	if (cells != null) {
		for (var i = 0; i < cells.length; i++) {
			if (this.model.isVertex(cells[i])) {
				var geo = this.getCellGeometry(cells[i]);
				if (result == null) {
					result = new mxRectangle(geo.x, geo.y, geo.width,
							geo.height);
				} else {
					result.add(geo);
				}
		...
		if (s2 > 0.1 && s2 < 8) {
			if (!keepOrigin) {
				this.view.translate.x = (bounds.x != null)
						? this.view.translate.x - bounds.x / s + border
						: border;
				this.view.translate.y = (bounds.y != null)
						? this.view.translate.y - bounds.y / s + border
						: border;
			}
...
mxGraph.prototype.scrollCellToVisible = function(cell, center) {
	...
	if (state != null) {
		...
		if (this.scrollRectToVisible(bounds)) {
			this.view
					.setTranslate(this.view.translate.x, this.view.translate.y);
		}
	}
};
...
mxGraph.prototype.isOrthogonal = function(edge, vertex) {
	...
	return tmp == mxEdgeStyle.ElbowConnector || tmp == mxEdgeStyle.SideToSide
...
mxGraph.prototype.getEdgeValidationError = function(edge, source, target) {
	...
	if (source != null && target != null) {
		...
		for (var i = 0; i < this.multiplicities.length; i++) {
			var err = this.multiplicities[i].check(this, edge, source, target,
					sourceOut, targetIn);
			if (err != null) {
				error += err;
			}
...
mxGraph.prototype.validateGraph = function(cell, context) {
	...
	if (this.model.isEdge(cell)) {
		warning += this.getEdgeValidationError(cell, this.model.getTerminal(
				cell, true), this.model.getTerminal(cell, false))
				|| '';
	} else {
...
mxGraph.prototype.isValidDropTarget = function(cell, cells, evt) {
	return cell != null
			&& ((this.isSplitEnabled() && this.isSplitTarget(cell, cells, evt)) || (!this.model
					.isEdge(cell) && (this.isSwimlane(cell) || (this.model
					.getChildCount(cell) > 0 && !this.isCellCollapsed(cell)))));
};
	...
	if (state != null) {
		...
		if (pts != null) {
			...
			for (var i = 1; i < pts.length; i++) {
				var next = pts[i];
				var dist = mxUtils
						.ptSegDistSq(pt.x, pt.y, next.x, next.y, x, y);
				if (dist <= t2) {
					return true;
				}
				pt = next;
...
mxGraph.prototype.getEdges = function(cell, parent, incoming, outgoing,
	...
	for (var i = 0; i < childCount; i++) {
		...
		if (isCollapsed || !this.isCellVisible(child)) {
			edges = edges
					.concat(this.model.getEdges(child, incoming, outgoing));
		}
	}
	...
	if (parent != null) {
		...
		for (var i = 0; i < childCount; i++) {
			...
			if (this.model.isVertex(cell) && this.isCellVisible(cell)) {
				var conns = this
						.getConnections(cell, (isolate) ? parent : null);
				var fanOut = 0;
				var fanIn = 0;
...
mxGraph.prototype.selectCells = function(vertices, edges, parent) {
	...
	var filter = mxUtils.bind(this, function(cell) {
		return this.view.getState(cell) != null
				...
				&& ((this.model.isVertex(cell) && vertices) || (this.model
						.isEdge(cell) && edges));
	});
...
function mxOutline(source, container) {
	this.source = source;
	this.outline = new mxGraph(container, source.getModel(),
	...
	source.getModel().addListener(mxEvent.CHANGE,
	...
	this.selectionBorder.dialect = (this.outline.dialect != mxConstants.DIALECT_SVG)
...
mxOutline.prototype.update = function(revalidate) {
	...
	this.bounds = new mxRectangle((t2.x - t.x) / scale3, (t2.y - t.y) / scale3,
			(container.clientWidth / scale2), (container.clientHeight / scale2));
...
mxOutline.prototype.mouseMove = function(sender, me) {
	if (this.active) {
		...
		if (this.index == null) {
			...
			bounds = new mxRectangle(this.bounds.x + dx, this.bounds.y + dy,
...
function mxMultiplicity(source, type, attr, value, min, max, validNeighbors,
		countError, typeError, validNeighborsAllowed) {
	this.source = source;
	this.type = type;
	this.attr = attr;
	this.value = value;
...
mxMultiplicity.prototype.check = function(graph, edge, source, target,
	...
	if ((this.source && this.checkType(graph, sourceValue, this.type,
			...
			|| (!this.source && this.checkType(graph, targetValue, this.type,
					this.attr, this.value))) {
...
mxLayoutManager.prototype.layoutCells = function(cells) {
	if (cells.length > 0) {
		...
		try {
			...
			this.fireEvent(new mxEventObject(mxEvent.LAYOUT_CELLS, 'cells',
					cells));
		} finally {
			model.endUpdate();
		}
			...
			try {
				for (var i = 0; i < cells.length; i++) {
					if (cells[i] != cell && this.isCellShiftable(cells[i])) {
						this.shiftCell(cells[i], dx, dy, x0, y0, right, bottom,
								fx, fy, this.isExtendParents()
...
mxSpaceManager.prototype.getCellsToShift = function(state) {
	...
	return graph.getCellsBeyond(state.x + ((down) ? 0 : state.width), state.y
			+ ((down && right) ? 0 : state.height), parent, right, down);
};
...
mxGraphHandler.prototype.start = function(x, y) {
	...
	if (this.bounds != null) {
		...
		if (this.shape.dialect == mxConstants.DIALECT_SVG) {
			...
			mxEvent.addListener(this.shape.node, 'mousemove', mxUtils.bind(
					this, function(evt) {
						var state = this.graph.getView().getState(this.target
								|| this.cell);
						this.graph.fireMouseEvent(mxEvent.MOUSE_MOVE,
								new mxMouseEvent(evt, state));
					}));
...
mxGraphHandler.prototype.mouseMove = function(sender, me) {
	if (!me.isConsumed() && this.cell != null && this.shape != null
		...
		if (this.shape.node.style.visibility == 'visible' || Math.abs(dx) > tol
				|| Math.abs(dy) > tol) {
...
mxPanningHandler.prototype.mouseUp = function(sender, me) {
	...
	if (this.active) {
		...
		if (dx < this.graph.tolerance && dy < this.graph.tolerance) {
			var cell = me.getCell();
			if (this.graph.isEnabled() && this.selectOnPopup && cell != null
...
mxCellMarker.prototype.intersects = function(state, me) {
	if (this.hotspotEnabled) {
		var point = mxUtils.convertPoint(this.graph.container, me.getX(), me
				.getY());
		return mxUtils.intersectsHotspot(state, point.x, point.y, this.hotspot,
...
mxConnectionHandler.prototype.init = function() {
	if (this.graph.container != null) {
		...
		var changeHandler = mxUtils.bind(this, function(sender) {
		...
		this.graph.getView().addListener(mxEvent.SCALE_AND_TRANSLATE,
				changeHandler);
		var drillHandler = mxUtils.bind(this, function(sender) {
			this.destroyIcons(this.icons);
		});
		...
		if (cell != null) {
			if (self.isConnecting()) {
				if (self.previous != null) {
					self.error = self.validateConnection(self.previous.cell,
							cell);
...
mxConnectionHandler.prototype.createIcons = function(state) {
	...
	if (image != null && state != null) {
		...
		mxEvent.addListener(icon.node, 'dblclick', mxUtils.bind(this, function(
				evt) {
		...
		mxEvent.addListener(icon.node, 'mousedown', mxUtils.bind(this,
				function(evt) {
					this.icon = icon;
					this.graph.fireMouseEvent(mxEvent.MOUSE_DOWN,
							new mxMouseEvent(evt, (this.currentState != null)
									? this.currentState
									: state));
				}));
		...
		mxEvent.addListener(icon.node, 'mouseup', mxUtils.bind(this, function(
				evt) {
			this.graph.fireMouseEvent(mxEvent.MOUSE_UP, new mxMouseEvent(evt,
					(this.currentState != null) ? this.currentState : state));
		}));
		icons.push(icon);
...
mxConnectionHandler.prototype.mouseDown = function(sender, me) {
	if (!me.isConsumed()
			...
			&& me.getHandle() == null) {
		this.start = mxUtils.convertPoint(this.graph.container, me.getX(), me
				.getY());
...
mxConnectionHandler.prototype.mouseMove = function(sender, me) {
	if (!me.isConsumed()
		...
		if (this.previous != null && this.start != null) {
			var view = this.graph.getView();
			var scale = view.scale;
			var pt = mxUtils.convertPoint(this.graph.container, me.getX(), me
					.getY());
			...
			if (state != null) {
				...
				if (targetPerimeter != null) {
					...
					var tmp = targetPerimeter(view.getPerimeterBounds(state,
							this.edgeState, false), this.edgeState, state,
							false, next);
					if (tmp != null) {
						current = tmp;
					}
			...
			if (sourcePerimeter != null) {
				var tmp = sourcePerimeter(view.getPerimeterBounds(
						this.previous, this.edgeState, true), this.edgeState,
...
mxConnectionHandler.prototype.insertEdge = function(parent, id, value, source,
		target) {
...
mxRubberband.prototype.mouseDown = function(sender, me) {
	if (!me.isConsumed()
			&& this.isEnabled()
			&& this.graph.isEnabled()
			&& (this.graph.isForceMarqueeEvent(me.getEvent()) || (me.getState() == null && me
					.getHandle() == null))) {
		this.start(me.getX(), me.getY());
		me.consume();
	} else {
	...
	if (this.sizers != null) {
		...
		if (this.singleSizer) {
			...
			if (this.sizers.length > 1) {
				...
				this.moveSizerTo(this.sizers[8], cx + s.absoluteOffset.x, cy
						+ s.absoluteOffset.y);
			}
		}
	}
...
mxEdgeHandler.prototype.init = function() {
			...
			function(evt) {
				...
				if (this.index != null) {
					...
					if (this.graph.isSwimlane(cell)
							&& this.graph.hitsSwimlaneContent(cell, pt.x, pt.y)) {
						cell = null;
					}
				}
				this.graph.fireMouseEvent(mxEvent.MOUSE_MOVE, new mxMouseEvent(
						evt, this.graph.getView().getState(cell)));
			}));
	mxEvent.addListener(this.shape.node, 'mouseup', mxUtils.bind(this,
			function(evt) {
				this.graph.fireMouseEvent(mxEvent.MOUSE_UP, new mxMouseEvent(
						evt, this.state));
			}));
	...
	mxEvent.addListener(this.labelShape.node, 'mousedown', mxUtils.bind(this,
			function(evt) {
				this.graph.fireMouseEvent(mxEvent.MOUSE_DOWN, new mxMouseEvent(
						evt, this.state, mxEvent.LABEL_HANDLE));
			}));
	this.redraw();
};
		...
		if (me.getX() != this.startX || me.getY() != this.startY) {
			if (this.error != null) {
				...
				if (this.marker.hasValidState()) {
					var edge = this.connect(edge,
							this.marker.getValidState().cell, this.isSource,
...
mxEdgeHandler.prototype.convertPoint = function(point, gridEnabled) {
	...
	var pstate = this.graph.getView().getState(this.graph.getModel()
...
mxEdgeHandler.prototype.connect = function(edge, terminal, isSource, isClone) {
	...
	try {
		...
		if (terminal == null) {
			...
			var current = this.abspoints[(isSource) ? 0 : this.abspoints.length
					- 1];
			var geo = model.getGeometry(edge).clone();
			geo.setTerminalPoint(new mxPoint((current.x - dx) / scale - tr.x,
...
mxEdgeHandler.prototype.redraw = function() {
	...
	this.label = new mxPoint(this.state.absoluteOffset.x,
			this.state.absoluteOffset.y);
	var bounds = new mxRectangle(this.label.x - s, this.label.y - s, 2 * s, 2
			* s);
	...
	if (pts != null) {
		for (var i = 1; i < this.bends.length - 1; i++) {
			if (this.abspoints[i] != null) {
				...
				this.bends[i].bounds = new mxRectangle(x - s, y - s, 2 * s, 2
						* s);
				this.bends[i].redraw();
...
mxEdgeHandler.prototype.drawPreview = function() {
	...
	mxUtils.repaintGraph(this.graph, this.shape.points[this.shape.points.length
			- 1]);
};
mxEdgeHandler.prototype.installListeners = function(bend, handle) {
	mxEvent.addListener(bend.node, 'mousedown', mxUtils.bind(this,
			function(evt) {
				this.graph.fireMouseEvent(mxEvent.MOUSE_DOWN, new mxMouseEvent(
						evt, this.state, handle));
			}));
	mxEvent.addListener(bend.node, 'mouseup', mxUtils.bind(this, function(evt) {
		this.graph.fireMouseEvent(mxEvent.MOUSE_UP, new mxMouseEvent(evt,
				this.state, handle));
	}));
};
...
mxTooltipHandler.prototype.reset = function(me, restart) {
	...
	if (restart && this.isEnabled() && me.getState() != null
		...
		this.thread = window.setTimeout(mxUtils.bind(this, function() {
			if (!this.graph.isEditing()
					&& !this.graph.panningHandler.isMenuShowing()) {
...
function mxCellHighlight(graph, highlightColor, strokeWidth) {
	if (graph != null) {
		...
		if (false || false || false) {
			...
			mxEvent.addListener(this.shape.node, 'mousedown', mxUtils.bind(
					this, function(evt) {
						graph.fireMouseEvent(mxEvent.MOUSE_DOWN,
								new mxMouseEvent(evt, this.state));
					}));
		...
		mxEvent.addListener(this.edgeShape.node, 'mousedown', mxUtils.bind(
				this, function(evt) {
					graph.fireMouseEvent(mxEvent.MOUSE_DOWN, new mxMouseEvent(
							evt, this.state));
				}));
		mxEvent.addListener(this.edgeShape.node, 'mousemove', mxUtils.bind(
				this, function(evt) {
	...
	while (item != null) {
		if (item.nodeName == 'add') {
			...
			if (condition == null || conditions[condition]) {
				...
				var row = this.addAction(menu, editor, as, icon, funct, action,
						cell, parent);
...
mxDefaultToolbar.prototype.init = function(container) {
	if (container != null) {
		...
		this.toolbar.addListener(mxEvent.SELECT, mxUtils.bind(this, function(
				sender, evt) {
...
mxDefaultToolbar.prototype.addPrototype = function(title, icon, ptype, pressed,
		insert) {
	var img = null;
	if (ptype == null) {
...
mxDefaultToolbar.prototype.insert = function(vertex, evt, target) {
	...
	if (graph.canImportCell(vertex)) {
		var pt = mxUtils
				.convertPoint(graph.container, evt.clientX, evt.clientY);
		if (graph.isSplitEnabled()
...
mxEditor.prototype.createSession = function() {
	...
	session.addListener(mxEvent.CONNECT, mxUtils.bind(this, function(sender,
			evt) {
		this.resetHistory();
	}));
};
...
mxEditor.prototype.execute = function(actionname, cell) {
	...
	if (action != null) {
		try {
			...
			mxUtils.error('Cannot execute ' + actionname + ': ' + e.message,
					280, true);
			throw e;
		}
	} else {
...
mxEditor.prototype.createGraph = function() {
	...
	graph.panningHandler.factoryMethod = mxUtils.bind(this, function(menu,
			cell, evt) {
		return this.createPopupMenu(menu, cell, evt);
	});
	graph.connectionHandler.factoryMethod = mxUtils.bind(this, function(source,
			target) {
...
mxEditor.prototype.installDblClickHandler = function(graph) {
	graph.addListener(mxEvent.DOUBLE_CLICK, mxUtils.bind(this, function(sender,
			evt) {
...
mxEditor.prototype.installChangeHandler = function(graph) {
	var listener = mxUtils.bind(this, function(sender, evt) {
		...
		for (var i = 0; i < changes.length; i++) {
			var change = changes[i];
			if (change.constructor == mxRootChange
					|| (change.constructor == mxValueChange && change.cell == this.graph.model.root)
					|| (change.constructor == mxCellAttributeChange && change.cell == this.graph.model.root)) {
				this.fireEvent(new mxEventObject(mxEvent.ROOT));
				break;
			}
		}
...
mxEditor.prototype.setStatusContainer = function(container) {
	if (this.status == null) {
		...
		this.addListener(mxEvent.SAVE, mxUtils.bind(this, function() {
			...
			this
					.setStatus((mxResources.get(this.lastSavedResource) || this.lastSavedResource)
							+ ': ' + tstamp);
		}));
		this.addListener(mxEvent.OPEN, mxUtils.bind(this, function() {
			this
					.setStatus((mxResources.get(this.currentFileResource) || this.currentFileResource)
							+ ': ' + this.filename);
		}));
		if (true) {
			new mxDivResizer(container);
...
mxEditor.prototype.postDiagram = function(url, data) {
	...
	mxUtils.post(url, this.postParameterName + '=' + data, mxUtils.bind(this,
			function(req) {
				this.fireEvent(new mxEventObject(mxEvent.POST, 'request', req,
						'url', url, 'data', data));
			}));
};
...
mxEditor.prototype.connect = function(urlInit, urlPoll, urlNotify, onChange) {
	...
	if (!mxClient.IS_LOCAL) {
		var session = new mxSession(this.graph.getModel(), urlInit, urlPoll,
				urlNotify);
		session.addListener(mxEvent.RECEIVE, mxUtils.bind(this, function(
				sender, evt) {
...
mxEditor.prototype.addVertex = function(parent, vertex, x, y) {
	...
	this.fireEvent(new mxEventObject(mxEvent.BEFORE_ADD_VERTEX, 'vertex',
	...
	try {
		...
		if (vertex != null) {
			...
			this.fireEvent(new mxEventObject(mxEvent.ADD_VERTEX, 'vertex',
					vertex));
		}
	} finally {
		model.endUpdate();
	...
	if (vertex != null) {
		...
		this.fireEvent(new mxEventObject(mxEvent.AFTER_ADD_VERTEX, 'vertex',
				vertex));
	}
	return vertex;
};
		...
		if (this.isReference(obj, name, value, true)) {
			...
			if (tmp == null) {
				mxLog.warn('mxObjectCodec.encode: No ID for '
						+ mxUtils.getFunctionName(obj.constructor) + '.' + name
						+ '=' + value);
				return;
			}
			value = tmp;
...
mxObjectCodec.prototype.writeComplexAttribute = function(enc, obj, attr, value,
		node) {
...
mxCodecRegistry.register(mxGenericChangeCodec(new mxCollapseChange(),
		'collapsed'));
mxCodecRegistry
		.register(mxGenericChangeCodec(new mxVisibleChange(), 'visible'));
