...
%><%@page import="java.util.HashMap"%><%@page
	import="java.io.OutputStream"%><%@ page import="javax.imageio.ImageIO"%><%@ page
	import="java.awt.image.BufferedImage"%><%@ page
	import="javax.imageio.stream.FileImageOutputStream"%><%@page
	import="java.util.Iterator"%><%@page import="java.io.File"%><%@ page
	import="com.fusioncharts.exporter.generators.ImageGenerator"%><%@ page
	import="com.fusioncharts.exporter.encoders.JPEGEncoder"%><%@ page
	import="com.fusioncharts.exporter.encoders.BasicEncoder"%><%@ page
	import="com.fusioncharts.exporter.beans.ChartMetadata"%><%@page
	import="com.fusioncharts.exporter.FusionChartsExportHelper"%><%@ page
	import="com.fusioncharts.exporter.ErrorHandler"%>
	<%@ page import="com.zznode.inms.fusioncharts.util.TableRender"%>
	<jsp:useBean
	id="exportBean" scope="request"
...
<%
	...
	if (!action.equals("download")) {//exportAction = save
		...
		String pathToSaveFolder = pathToWebAppRoot + File.separator
		...
		if (saveFile.exists()) {
			...
			if (!FusionChartsExportHelper.OVERWRITEFILE) {
				if (FusionChartsExportHelper.INTELLIGENTFILENAMING) {
					noticeMessage += ErrorHandler
							.getErrorForCode("W514");
					completeFilePath = FusionChartsExportHelper
							.getUniqueFileName(
									completeFilePathWithoutExt,
									extension);
					File tempFile = new File(completeFilePath);
					fileName = tempFile.getName();
					noticeMessage += ErrorHandler
							.getErrorForCode("W515")
							+ fileName;
		...
		if (FusionChartsExportHelper.HTTP_URI.endsWith("/")) {
			pathToDisplay = FusionChartsExportHelper.HTTP_URI
					+ fileName;
		}

		meta_values = exportBean.getMetadataAsQueryString(
		...
		try {
			...
			TableRender tr = (TableRender)request.getSession().getAttribute("mylegend");
			request.getSession().removeAttribute("mylegend");
			java.awt.Graphics2D g1 = chartImage.createGraphics();
			tr.install(g1);
			
			System.out.println("chart height="+metadata.getHeight()+", tr height="+tr.getTableHeight());
	        int height = metadata.getHeight()+tr.getTableHeight();
	        int width = metadata.getWidth();
			BufferedImage imageB = new BufferedImage(width,height,BufferedImage.TYPE_INT_RGB);
			java.awt.Graphics2D g2 = imageB.createGraphics();
			g2.setColor(java.awt.Color.white);
			g2.fillRect(0,0,width,height);
			g2.drawImage(chartImage, 0, 0, null);
			g2.translate(0, chartImage.getHeight());
			
			tr.paint(g2);
			
			g2.translate(-10, -chartImage.getHeight());
			
			jpegEncoder.encode(imageB, fios);
		} catch (Throwable e) {
		...
		if (err_warn_Codes.indexOf("E") < 0) {
			...
			out.print(meta_values + noticeMessage
					+ "&statusCode=1&statusMessage=successful");
		}

	} else {//exportAction = download
		...
		try {
			...
			out.print(meta_values
					+ noticeMessage
					+ ErrorHandler.buildResponse(err_warn_Codes
							.toString(), isHTML));
			return;
		}
		chartImage = null;
