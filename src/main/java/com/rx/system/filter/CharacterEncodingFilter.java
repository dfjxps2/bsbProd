...
import com.rx.system.util.RequestUtils;
...
public class CharacterEncodingFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
		...
		String uri=RequestUtils.requestToServerUriStringBuffer(request).toString();
		
		
		request.setCharacterEncoding("UTF-8");
		
		if (uri.indexOf("Ajax") != -1 ||uri.indexOf("ExcelService") != -1) {
			request.setCharacterEncoding("UTF-8");
		} else if (this.forceEncoding || request.getCharacterEncoding() == null) {
