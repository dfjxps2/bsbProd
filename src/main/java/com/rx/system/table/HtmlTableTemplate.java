 ...
 *
 */
public class HtmlTableTemplate extends AbstractTableTemplate {
		...
		if(this.header != null) {
			...
			for (int j = 0; j < firstRow.length; j++) {
				if(this.add_serial && this.serial_number == j) {
					sb.append("<td width=\"40\" align=\"center\">序号</td>");
				}
				if(this.useCheck && j == this.checkbox_index)
					sb.append("<td width=\"40\" align=\"center\">#master_checkbox</td>");
				sb.append("<td width=\""+ this.widths[j] +"\" align=\""+ this.aligns[j] +"\">"+ firstRow[j] +"</td>");
			}
	...
	protected void writeNomalTable(StringBuffer sb) {
		for (int i = 0; i < this.dataList.size(); i++) {
			...
			sb.append("<tr id=\""+this.getRowId(map)+"\">");
	...
	protected void writeLeftTableData(StringBuffer sb, Tree tree, Map<String, String> nodeAttrMap, List<TreeNode> preTreeNodeList) { 
		...
		if(children.size() == 0) {
			...
			if(this.filter_blank && dataMap.isEmpty())
				return;
			//叶子节点
			sb.append("<tr id=\""+preIds.replaceFirst(";", "").replaceAll(";", "@")+"\">");
			...
			for (int i = 0; i < preTreeNodeList.size() + 1; i++) {
				...
				if(entry == null || "".equals(entry)) {
					...
					if(href != null) {
						...
						if(this.href_drill)
							sb.append("<a href="+href+" >" + node.getNodeName() + "</a>");
						else
							sb.append("<a href=javascript:void(0) onclick="+href+" >" + node.getNodeName() + "</a>");
					}else {
			...
			for (int i = 0; i < this.data_keys.length; i++) {
				...
				if(href != null) {
					...
					if(this.href_drill)
						sb.append("<a href="+href+" >"+ getFormatString(getStringValue(dataMap, this.data_keys[i]), this.format[i+this.leftTreeIndex]) +"</a>");
					else
						sb.append("<a href=javascript:void(0) onclick="+href+" >"+ getFormatString(getStringValue(dataMap, this.data_keys[i]), this.format[i+this.leftTreeIndex]) +"</a>");
				}else {
	...
	protected void writeLeftTreeData(StringBuffer sb, List<Map<String, Object>> list, Tree tree, boolean isTop) {
		...
		sb.append("<tr id=\""+this.getRowId(dataMap)+"\">");
	...
	protected String getCellCode(int colIndex, Map<String, Object> map, String key, String type) {
		...
		if(href != null) {
			...
			if(this.href_drill)
				return "<td><a href="+href+" >"+ this.getFormatString(getStringValue(map, key), type) +"</a></td>";
			else
				return "<td><a href=javascript:void(0) onclick="+href+" >"+ this.getFormatString(getStringValue(map, key), type) +"</a></td>";
		}
		return "<td>" + this.getFormatString(getStringValue(map, key), type) + "</td>";
	}
