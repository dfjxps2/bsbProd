...
public class ValidatorFactoryForNoneException implements ValidatorFactory {
    ...
    private static Logger LOG = LoggerFactory.getLogger(ValidatorFactoryForNoneException.class);
    protected ObjectFactory objectFactory;
    protected ValidatorFileParser validatorFileParser;

    @Inject
    public ValidatorFactoryForNoneException(@Inject ObjectFactory objectFactory, @Inject ValidatorFileParser parser) {
    ...
    private void parseValidators() {
        ...
        try {
            ...
            Iterator<URL> urls = ClassLoaderUtil.getResources("", ValidatorFactoryForNoneException.class, false);
            while (urls.hasNext()) {
                URL u = urls.next();
                try {
...
//            throw new ConfigurationException("Unable to parse validators", e);
    ...
    private void retrieveValidatorConfiguration(String resourceName) {
        InputStream is = ClassLoaderUtil.getResourceAsStream(resourceName, ValidatorFactoryForNoneException.class);
        if (is != null) {
